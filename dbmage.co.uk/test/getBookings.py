...
#            print('No upcoming events found.')
            jsonout[calendar_ids[calendar_id]] = 'none'
            continue
        jsonout[calendar_ids[calendar_id]] = []
        for event in events:
            if 'dateTime' not in event['start']:
                continue
#            print("%s - %s - %s" % ( event['start']['dateTime'], desireddate, event['end']['dateTime']))
            jsonout[calendar_ids[calendar_id]].append({'start' : event['start']['dateTime'], 'end' : event['end']['dateTime']})
...
#                print("%-10s %-30s %-20s %-20s %s" % (calendar_ids[calendar_id], event['creator']['displayName'], start, end, event['summary']))
        if len(jsonout[calendar_ids[calendar_id]]) < 1:
            jsonout[calendar_ids[calendar_id]].append('none')
